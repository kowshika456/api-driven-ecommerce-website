...
from .models import Customer, Product, Cart, OrderPlaced, subscriber, search
...
def topwear(request, datas=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user)) 
	if datas==None :
			topwears = Product.objects.filter(category='TW')
	elif datas == 'Safira' or datas == 'Bajaj':
			topwears = Product.objects.filter(category='TW').filter(brand=datas)
	elif datas == 'below':
...
def bottomwear(request, datas=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user)) 
	if datas==None :
			bottomwears = Product.objects.filter(category='BW')
	elif datas == 'Lee' or datas == 'Spykar':
			bottomwears = Product.objects.filter(category='BW').filter(brand=datas)
	elif datas == 'below':
...
def laptop(request, datas=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user)) 
	if datas==None :
			laptops = Product.objects.filter(category='LP')
	elif datas == 'HP' or datas == 'ASUS':
			laptops = Product.objects.filter(category='LP').filter(brand=datas)
	elif datas == 'below':
			laptops = Product.objects.filter(category='LP').filter(discounted_price__lt=30000)
	elif datas == 'above':
			laptops = Product.objects.filter(category='LP').filter(discounted_price__gt=30000)
...
def logout_view(request):
    ...
    return redirect('home')  # Redirect to home page or any desired URL
...
def payment(request):
    if request.method == 'POST':
        ...
        if cardnumber and expirationdate:
            if not subscriber.objects.filter(cardnumber=cardnumber).exists():
                    subscriber.objects.create(cardnumber=cardnumber, expirationdate=expirationdate, email=email, mobile=mobile)
                    messages.success(request, 'Data Fetched successful.')
...
def excel(request):
    members=subscriber.objects.all()
    return render(request,'app/excel.html',{'data':members})

def search_view(request):
	search_obj = request.GET.get('search')
	result = Product.objects.filter(category=search_obj)
	if result:
		results=result
	else:
		results=False
	return render(request, "app/search.html",{'product_list':results} )
...
class DataManipulationView(APIView):
    ...
    def post(self, request):
        PostData = DataSerializers(data = request.data)
    ...
    def get(self, request, cid=None):
        if cid == None:
            getAllData = Product.objects.all()

            formatChange = DataSerializers(getAllData, many=True).data

            return Response(formatChange)
        else:
            getSingleData = Product.objects.get(id=cid)

            FormatChanging = DataSerializers(getSingleData, partial=True).data

            return Response(FormatChanging)
        
    def patch(self, request, cid):
        get = Product.objects.get(id=cid)

        getData = DataSerializers(get, data = request.data)
    ...
    def delete(self, request, cid):
        getData = Product.objects.get(id=cid)
